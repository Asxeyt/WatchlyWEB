...
WatchlyRelease.EnsureSignatureIntegrity();
...
builder.Services.AddScoped<CatalogSources>();
...
static void EnsureCatalogTables(AppDbContext db)
{
    // EnsureCreated does not add new tables to an existing v1.x database.
    if (db.Database.IsNpgsql())
    {
        db.Database.ExecuteSqlRaw("""
            CREATE TABLE IF NOT EXISTS "CatalogWorks" (
                ...
                "AltName" character varying(200) NOT NULL,
                "PosterUrl" character varying(600) NOT NULL,
                "Tur" character varying(240) NOT NULL,
                "Konu" character varying(3000) NOT NULL,
                "Puan" character varying(80) NOT NULL,
                "Fiyat" character varying(80) NOT NULL,
                "YayinTarihi" character varying(80) NOT NULL,
                "Yapimci" character varying(240) NOT NULL,
                "UpdatedAt" timestamp with time zone NOT NULL
            );
            """);
        db.Database.ExecuteSqlRaw("""
            CREATE TABLE IF NOT EXISTS "CatalogExternalIds" (
                "Id" integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                "CatalogWorkId" integer NOT NULL REFERENCES "CatalogWorks"("Id") ON DELETE CASCADE,
                "Kategori" integer NOT NULL,
                "Source" character varying(40) NOT NULL,
                "ExternalId" character varying(100) NOT NULL
            );
            """);
        db.Database.ExecuteSqlRaw("CREATE INDEX IF NOT EXISTS \"IX_CatalogWorks_Kategori_NormalizedTitle_ReleaseYear\" ON \"CatalogWorks\" (\"Kategori\", \"NormalizedTitle\", \"ReleaseYear\");");
        db.Database.ExecuteSqlRaw("CREATE UNIQUE INDEX IF NOT EXISTS \"IX_CatalogExternalIds_Kategori_Source_ExternalId\" ON \"CatalogExternalIds\" (\"Kategori\", \"Source\", \"ExternalId\");");
    }
...
static void ApplySqliteLegacyFixes(AppDbContext db)
{
    ...
    using (var reader = cmd.ExecuteReader())
    {
        while (reader.Read())
        {
            ...
            if (string.Equals(reader["name"]?.ToString(), "CatalogWorkId", StringComparison.OrdinalIgnoreCase))
            {
                hasCatalogWorkId = true;
            }
    ...
    if (!hasCatalogWorkId)
    {
        db.Database.ExecuteSqlRaw("ALTER TABLE MedyaOgeleri ADD COLUMN CatalogWorkId INTEGER NULL;");
    }
    if (!hasPosterUrl)
    {
...
static void ApplyPostgresLegacyFixes(AppDbContext db)
{
    db.Database.ExecuteSqlRaw("""
        ...
        ADD COLUMN IF NOT EXISTS "CatalogWorkId" integer NULL,
